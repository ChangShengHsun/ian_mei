...
Two-fold (STARE has no official split, so this declares its own) x two targets
x two seeds = 8 runs of the plain BCE + Dice baseline. The loss is held fixed
because the variable under test is the annotation, not the objective.

  python exp/train_stare.py            # all 8, resumable
  python exp/train_stare.py ah_f0_s0   # one run

~45 min per run, ~6 h total on 6 cores.
"""
import csv
import sys
import time
...
def main() -> None:
    ...
    names = [f"{t}_f{f}_s{s}" for s in SEEDS for f in FOLDS for t in TARGETS]
