...
The predictions come from DRIVE-trained K_focal_aug, applied across datasets
in the repo's usual transfer protocol. They are not expected to be good on
VessMAP -- that is fine and arguably better: the question is whether the
CORRECTION's geometry is dataset-invariant, not whether the segmentation is.
...
def main() -> None:
    ...
    rule = dict(rules_module.rules())[direction_ceiling.RULE]
    ...
    for name, items in datasets().items():
        ...
        print(f"{name}: {len(items)} images, width {width:.2f} px, "
        ...
        for seed in SEEDS:
            ...
            for item, geo, field in zip(items, geometry, fields):
                ...
                def record(source, along, across, mask):
                    rows.append({
                        **common, "source": source, "along": along,
                        ...
                        "dice": round(link_ceiling.dice(mask, geo["truth"]),
                                      5),
                        "fg": int(mask.sum())})
                ...
                for radius in direction_ceiling.ISOTROPIC:
                    ...
                    record("isotropic", radius, radius,
                           anisotropic.isotropic_dilation(pred, radius * width)
                           & item["fov"])
            ...
            print(f"  {run} done on {name}", flush=True)
